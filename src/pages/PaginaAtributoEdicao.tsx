...
const PaginaBase: React.FC = () => {
  ...
  const buscaDados = async () => {
    try {
      ...
      let imgAtributo = "";
      imgAtributo = atributoPlaceholder;
      await executarAcaoSQL(async (db: SQLiteDBConnection | undefined) => {
        const comandoSQL = `
            SELECT COALESCE(nome, 'Casca Vazia') as nome, 
                COALESCE(observacao, 'Uma casca vazia esperando para ser algo') as descricao, 
                COALESCE(imagem, '${atributoPlaceholder}') as imagem,
                COALESCE(xp, 0) as xp
            FROM ATRIBUTO WHERE ID = ? LIMIT 1`;
        const resultado = await db?.query(comandoSQL, [idEdicao]);
  ...
  const selecionaImagem = async () => {
    try {
      ...
      await executarAcaoSQL(async (db: SQLiteDBConnection | undefined) => {
        await db?.query(`UPDATE ATRIBUTO SET imagem = ? WHERE ID = ?`, [
          base64Imagem,
          idEdicao,
        ]);
